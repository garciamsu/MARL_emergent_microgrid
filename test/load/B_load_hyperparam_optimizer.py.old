#!/usr/bin/env python3
"""
load_hyperparam_optimizer.py
================================

Hyperparameter optimizer for LoadAgent reward function. Adapted from the
battery implementation; paths and names adjusted for load.
"""
...
DATASET_FILE = os.path.join(PROJECT_ROOT, "test/load/output/reward_load.csv")
...
def load_load_dataset(csv_path: str) -> pd.DataFrame:
    # Load agent-specific CSV used by test/load (delimiter ';')
    df = pd.read_csv(csv_path, delimiter=';', encoding="utf-8")
    required = ["soc_idx", "demand_idx", "renewable_potential_idx", "action"]
    for c in required:
        if c not in df.columns:
            raise ValueError(f"Missing column: {c}")
    df["dP"] = df["renewable_potential_idx"] - df["demand_idx"]
    return df


def _build_reward_instance(params: Dict[str, float]):
    cls = core_rewards.DefaultLoadReward
...
def compute_kpis_from_params(df: pd.DataFrame, params: Dict[str, float]) -> Dict[str, float]:
    ...
    class _Env:
        def __init__(self):
            # default market price if not provided elsewhere
            self.price = 1.0
    ...
    for _, row in df.iterrows():
        ...
        state_tuple = (int(row.soc_idx), int(row.demand_idx), int(row.renewable_potential_idx))
        try:
...
def main():
    ...
    df = load_load_dataset(DATASET_FILE)
