...
def generate_reports(input_file: str, output_dir: Path):
    """
    Generates analytical reports and visualizations based on the LoadAgent's reward output.
    ...
    plt.title("Reward Boxplot by Action (LoadAgent)")
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(output_dir / "boxplot_reward_by_action_load.png")
    plt.close()

    # Plot: histogram of rewards per action
    plt.figure(figsize=(10, 6))
    sns.histplot(data=df, x='reward', hue='action', multiple='stack', bins=50)
    plt.title("Reward Histogram by Action (LoadAgent)")
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(output_dir / "histogram_rewards_action_load.png")
    plt.close()

    # Plot: heatmap of average rewards by renewable_potential and battery SoC
    for comfort in df['comfort_idx'].unique():
        subset = df[df['comfort_idx'] == comfort]
        ...
        plt.tight_layout()
        plt.savefig(output_dir / f"heatmap_reward_comfort_{comfort}.png")
        plt.close()

    # Table: summary of reward statistics per action
    summary = df.groupby('action')['reward'].agg(['mean', 'std', 'min', 'max', 'count']).reset_index()
    summary.to_csv(output_dir / "reward_summary_by_action_load.csv", index=False, encoding='utf-8')

    # Plot: global reward distribution
    plt.figure(figsize=(10, 6))
    sns.histplot(df['reward'], bins=60, kde=True)
    plt.title("Global Reward Distribution (LoadAgent)")
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(output_dir / "reward_distribution_load.png")
    plt.close()

    # Plot: rolling mean of reward
    df_sorted = df.sort_values(by=['battery_soc_idx', 'renewable_potential_idx', 'action'])
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(output_dir / "rolling_mean_reward_load.png")
    plt.close()
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(output_dir / "action_frequency_load.png")
    plt.close()

    print(f"✅ Reports successfully generated in: {output_dir}")

if __name__ == "__main__":
    input_file = Path(__file__).parent / 'output' / 'reward_load.csv'
