...
Optimizador de hiperparámetros para la función de recompensa del agente Battery.
...
- /test/battery/output/fitness_convergence.svg
...
# Rutas
DATASET_FILE = os.path.join(PROJECT_ROOT, "test/battery/input/Battery_Agent_Reward_Table.csv")
...
SOC_MAX = 4  # Valor máximo de SOC discretizado
ALL_ACTIONS = [0, 1, 2]  # 0: inactivo, 1: cargar, 2: descargar
...
def load_dataset(csv_path: str) -> pd.DataFrame:
    ...
    required = ["renewable_idx", "demand_power_idx", "battery_soc_idx", "action"]
    for col in required:
        if col not in df.columns:
            raise ValueError(f"Falta la columna requerida: {col}")
    
    # Calcular delta_p_interno (excedente/déficit antes de la batería)
    df["delta_p_interno"] = df["renewable_idx"] - df["demand_power_idx"]
    
    print(f"[INFO] Dataset cargado: {len(df)} filas, {len(df.groupby(['renewable_idx', 'demand_power_idx', 'battery_soc_idx']))} estados únicos")
    
    return df


def state_transform_battery(state_dict: Dict, row: pd.Series) -> Dict:
    """
    ...
    state_dict['soc_max'] = SOC_MAX
    return state_dict


def compute_fitness_battery(df: pd.DataFrame, params: Dict[str, float]) -> float:
    """
    ...
    total_margin = compute_total_margin(
        df=df,
        params=params,
        reward_class=DefaultBatteryReward,
        state_columns=state_columns,
        action_column='action',
        get_correct_action_fn=get_correct_action_battery,
        all_actions=ALL_ACTIONS,
        env_extra_attrs=env_attrs,
        agent_extra_attrs=agent_attrs,
        state_transform_fn=state_transform_battery,
    )
...
def random_search(df: pd.DataFrame, budget: int, log_path: str) -> pd.DataFrame:
    ...
    for _ in range(budget):
        ...
        fitness = compute_fitness_battery(df, params)
...
def main():
    print("=" * 60)
    print(" OPTIMIZADOR DE HIPERPARÁMETROS - BATTERY AGENT")
    print("=" * 60)
    ...
    for idx, (i, row) in enumerate(best.iterrows(), 1):
        print(f"{idx}) psi={row.psi:.3f}, sigma={row.sigma:.3f}, nu={row.nu:.3f}, "
              f"beta={row.beta:.3f}, xi={row.xi:.3f}, mu={row.mu:.3f} | "
