...
# -*- coding: utf-8 -*-
"""
B_solar_hyperparam_optimizer.py
================================

Optimizador de hiperparámetros para la función de recompensa del agente Solar.
...
Inputs
------
- /test/solar/input/Solar_Agent_Reward_Table.csv
...
# Rutas
DATASET_FILE = os.path.join(PROJECT_ROOT, "test/solar/input/Solar_Agent_Reward_Table.csv")
...
def load_dataset(csv_path: str) -> pd.DataFrame:
    ...
    print(f"[INFO] Dataset cargado: {len(df)} filas, {len(df.groupby(['solar_idx', 'demand_idx', 'renewable_idx']))} estados únicos")
    
    return df


def state_transform_solar(state_dict: Dict, row: pd.Series) -> Dict:
    """
    Transforma el estado para incluir variables derivadas necesarias
    para determinar la acción correcta.
    """
    state_dict['solar_potential_idx'] = row['solar_idx']
    return state_dict


def compute_fitness_solar(df: pd.DataFrame, params: Dict[str, float]) -> float:
    """
    ...
    # Columnas de estado
    state_columns = ['solar_idx', 'demand_idx', 'renewable_idx']
    
    # Calcular margen total
    total_margin = compute_total_margin(
        df=df,
        params=params,
        reward_class=DefaultSolarReward,
        state_columns=state_columns,
        action_column='action',
        get_correct_action_fn=get_correct_action_solar,
        all_actions=ALL_ACTIONS,
        env_extra_attrs=env_attrs,
        agent_extra_attrs=agent_attrs,
        state_transform_fn=state_transform_solar,
    )
...
def random_search(df: pd.DataFrame, budget: int, log_path: str) -> pd.DataFrame:
    ...
    for _ in range(budget):
        ...
        fitness = compute_fitness_solar(df, params)
...
def main():
    print("=" * 60)
    print(" OPTIMIZADOR DE HIPERPARÁMETROS - SOLAR AGENT")
    print("=" * 60)
