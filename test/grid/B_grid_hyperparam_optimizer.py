...
Optimizador de hiperparámetros para la función de recompensa del agente Grid.
...
Outputs
-------
- /test/grid/output/hyperparam_optimization_log.csv
- /test/grid/output/fitness_convergence.svg
...
# Rutas
DATASET_FILE = os.path.join(PROJECT_ROOT, "test/grid/input/GridAgent_Reward_Table.csv")
...
def load_dataset(csv_path: str) -> pd.DataFrame:
    ...
    required = ["battery_soc_idx", "renewable_idx", "demand_power_idx", "action"]
    for col in required:
    ...
    print(f"[INFO] Dataset cargado: {len(df)} filas, {len(df.groupby(['battery_soc_idx', 'renewable_idx', 'demand_power_idx']))} estados únicos")
    
    return df


def state_transform_grid(state_dict: Dict, row: pd.Series) -> Dict:
    """
...
def compute_fitness_grid(df: pd.DataFrame, params: Dict[str, float]) -> float:
    """
    ...
    total_margin = compute_total_margin(
        df=df,
        params=params,
        reward_class=DefaultGridReward,
        state_columns=state_columns,
        action_column='action',
        get_correct_action_fn=get_correct_action_grid,
        all_actions=ALL_ACTIONS,
        env_extra_attrs=env_attrs,
        agent_extra_attrs=agent_attrs,
        state_transform_fn=state_transform_grid,
    )
...
def random_search(df: pd.DataFrame, budget: int, log_path: str) -> pd.DataFrame:
    ...
    for _ in range(budget):
        ...
        fitness = compute_fitness_grid(df, params)
...
def main():
    print("=" * 60)
    print(" OPTIMIZADOR DE HIPERPARÁMETROS - GRID AGENT")
    print("=" * 60)
    ...
    for idx, (i, row) in enumerate(best.iterrows(), 1):
        print(f"{idx}) psi={row.psi:.3f}, sigma={row.sigma:.3f}, nu={row.nu:.3f}, xi={row.xi:.3f} | "
