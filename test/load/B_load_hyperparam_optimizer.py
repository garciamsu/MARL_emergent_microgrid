...
# -*- coding: utf-8 -*-
"""
B_load_hyperparam_optimizer.py
===============================

Optimizador de hiperparámetros para la función de recompensa del agente Load.
...
Inputs
------
- /test/load/input/LoadAgent_Reward_Table.csv

Outputs
-------
- /test/load/output/hyperparam_optimization_log.csv
- /test/load/output/fitness_convergence.svg
...
# Rutas
DATASET_FILE = os.path.join(PROJECT_ROOT, "test/load/input/LoadAgent_Reward_Table.csv")
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "test/load/output/")
...
PARAM_BOUNDS: Dict[str, Tuple[float, float]] = {
    ...
    "psi":   (0.1, 5.0),
    "nu":    (0.1, 5.0),
    "beta":  (-2.0, 2.0),  # Puede ser negativo según la función
}

# Configuración específica del agente
COMFORT_THRESHOLD = 4  # Umbral de precio por defecto
ALL_ACTIONS = [0, 1]  # 0: apagar, 1: encender
...
def load_dataset(csv_path: str) -> pd.DataFrame:
    ...
    required = ["battery_soc_idx", "demand_idx", "renewable_idx", "price", "action"]
    for col in required:
        if col not in df.columns:
            raise ValueError(f"Falta la columna requerida: {col}")
    
    # Calcular delta_p_interno (excedente/déficit interno)
    df["delta_p_interno"] = df["renewable_idx"] - df["demand_idx"]
    
    print(f"[INFO] Dataset cargado: {len(df)} filas, {len(df.groupby(['battery_soc_idx', 'demand_idx', 'renewable_idx', 'price']))} estados únicos")
    
    return df


def state_transform_load(state_dict: Dict, row: pd.Series) -> Dict:
    """
    Transforma el estado para incluir variables derivadas necesarias
    para determinar la acción correcta.
    """
    state_dict['delta_p_interno'] = row['delta_p_interno']
    state_dict['soc_idx'] = row['battery_soc_idx']
    state_dict['comfort_threshold'] = COMFORT_THRESHOLD
    return state_dict


def compute_fitness_load(df: pd.DataFrame, params: Dict[str, float]) -> float:
    """
    ...
    # Columnas de estado
    state_columns = ['battery_soc_idx', 'demand_idx', 'renewable_idx', 'price']
    
    # Calcular margen total
    total_margin = compute_total_margin(
        df=df,
        params=params,
        reward_class=DefaultLoadReward,
        state_columns=state_columns,
        action_column='action',
        get_correct_action_fn=get_correct_action_load,
        all_actions=ALL_ACTIONS,
        env_extra_attrs=env_attrs,
        agent_extra_attrs=agent_attrs,
        state_transform_fn=state_transform_load,
    )
...
def random_search(df: pd.DataFrame, budget: int, log_path: str) -> pd.DataFrame:
    ...
    for _ in range(budget):
        ...
        fitness = compute_fitness_load(df, params)
...
def main():
    print("=" * 60)
    print(" OPTIMIZADOR DE HIPERPARÁMETROS - LOAD AGENT")
    print("=" * 60)
    ...
    for idx, (i, row) in enumerate(best.iterrows(), 1):
        print(f"{idx}) sigma={row.sigma:.3f}, psi={row.psi:.3f}, nu={row.nu:.3f}, beta={row.beta:.3f} | "
              f"margen={row.margin_total:.2f}")
    print("-" * 60)
